...
export class HistoryFilePage {
  ...
  initData() {
    this.babb.getHistoryFile(this.unit.unitOid).then(
      res => {
        this.fileList = res;
  ...
  onClickSearch() {
    ...
    this.fileList.forEach((item, index, list)=>{
      ...
      keyList.forEach((key, index, list)=>{
        if(item.FILE_NAME.match(key.trim())){
  ...
  fileOpen(fileName) {
    ...
    this.fileOpener.open(this.config.DB_HISTORY_FILE_LOCATION + fileName.FILE_NAME, mineType).then(
      data => {
